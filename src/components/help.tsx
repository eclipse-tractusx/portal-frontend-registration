                        ...
                        <Row>
                                ...
                                <Nav>
                                    <div ref={headerRef}>
                                        {sectionRefs.map((item, index) => {
                                            return <Nav.Link key={index} className={`${visibleSection === item.section ? "selected" : ""} help-menu`}
                                                onClick={() => scrollTo(item.ref)}>{t(`helpLink.link${index + 1}`)}</Nav.Link>
                                        })}
                                    </div>
                                </Nav>
                            </div>
                            ...
                            <div className="col-sm-9 offset-sm-3 data-section">
                                <div className="navContainer">
                                    {sectionRefs.map((item, index) => {
                                        return <div key={index} id={item.section} ref={item.ref}>
                                            <div>{t(`${item.section}.questionHeadline`)}</div>
                                            <div>{t(`${item.section}.question`)}</div>
                                            <div className='answer'>{t(`${item.section}.answer`)}</div>
                                        </div>
                                    })}
                                </div>
                            </div>
